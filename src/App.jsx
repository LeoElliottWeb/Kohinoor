...
function RemoteVideo({ stream, email, allKnownUsers }) {
    ...
    useEffect(() => {
        ...
        console.log("[RemoteVideo] Setting srcObject for:", email);
...
function ChatApp({ user, onLogout }) {
    ...
    useEffect(() => {
        const fetchMembers = async () => {
            ...
            } catch (err) { console.error("Exception fetching profiles:", err); }
    ...
    useEffect(() => {
        if (localVideoRef.current && localMediaStream) {
            ...
            localVideoRef.current.play().catch(e => { if (e.name !== 'AbortError') console.error("Local video play error:", e); });
        }
    }, [localMediaStream]);
    ...
    useEffect(() => {
        ...
        const fetchHistory = async () => {
            ...
            const allMessages = [...(sent.data || []), ...(received.data || [])].sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
    ...
    const createPeerConnection = (targetEmail) => {
        ...
        if (!pendingCandidatesRef.current[targetEmail]) pendingCandidatesRef.current[targetEmail] = [];
        ...
        pc.onicecandidate = (event) => {
            if (event.candidate && channelRef.current) {
                channelRef.current.send({ type: 'broadcast', event: 'webrtc-ice-candidate', payload: { targetEmail, candidate: event.candidate, sender: userEmail } });
            }
        };
        ...
        pc.onconnectionstatechange = () => {
            ...
            if (pc.connectionState === 'connected') {
                ...
                console.log("[WebRTC] ❌ Connection failed for:", targetEmail);
                cleanupPeer(targetEmail);
            } else if (pc.connectionState === 'disconnected') {
                console.log("[WebRTC] ⚠️ Disconnected from:", targetEmail, "- waiting for reconnection...");
                // DON'T immediately clean up on disconnect - wait for potential reconnection
                setTimeout(() => {
                    const currentPc = peersRef.current[targetEmail];
                    if (currentPc && currentPc.connectionState === 'disconnected') {
                        console.log("[WebRTC] Still disconnected after timeout, cleaning up:", targetEmail);
                        cleanupPeer(targetEmail);
                    }
        ...
        pc.ontrack = (event) => {
            ...
            if (event.streams && event.streams[0]) {
                ...
                setRemoteStreams(prev => ({ ...prev, [targetEmail]: remoteStream }));
            }
        };
    ...
    const cleanupPeer = (email) => {
        ...
        // Only end call if no peers AND we're not in the middle of ending
        if (Object.keys(peersRef.current).length === 0 && inCallRef.current && !isEndingCallRef.current) {
    ...
    const endVoiceCall = (broadcast = true) => {
        if (isEndingCallRef.current) return; // Prevent double-calling
        console.log("[WebRTC] Ending call, broadcast:", broadcast);
        ...
        // Reset the ending flag after state updates
    ...
    const startCall = async (targetEmail) => {
        ...
        try {
            ...
            channelRef.current.send({ type: 'broadcast', event: 'webrtc-offer', payload: { targetEmail, offer: pc.localDescription, sender: userEmail } });
            setInVoiceCall(true);
        } catch (err) {
    ...
    const acceptCall = async () => {
        ...
        try {
            ...
            channelRef.current.send({ type: 'broadcast', event: 'webrtc-answer', payload: { targetEmail: senderEmail, answer: pc.localDescription, sender: userEmail } });
    ...
    useEffect(() => {
        if (!userEmail) return;
        const channel = supabase.channel('totalrecall-global', { config: { presence: { key: `user_${userEmail}` } } });
            ...
            for (const key in state) {
                ...
                if (presences && presences.length > 0) {
                    ...
                    if (presence && presence.email && presence.email !== userEmail) {
                        if (!activeUsers.some(u => u.email === presence.email)) activeUsers.push({ email: presence.email, presenceKey: key });
                    }
                }
            }
        ...
        channel.on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, payload => {
            ...
            if ((newMsg.sender_email === currentContact && newMsg.receiver_email === userEmail) || (newMsg.sender_email === userEmail && newMsg.receiver_email === currentContact)) {
                setChatMessages(prev => { if (prev.find(m => m.id === newMsg.id)) return prev; return [...prev, newMsg]; });
            }
        });
        ...
        channel.on('broadcast', { event: 'webrtc-offer' }, async ({ payload }) => {
            ...
            // IMPORTANT: If we're already in a call OR just ended one, accept automatically
            if (localStreamRef.current) {
                console.log("[WebRTC] Auto-accepting - we have media ready");
                try {
                    const pc = createPeerConnection(payload.sender);
                    await pc.setRemoteDescription(new RTCSessionDescription(payload.offer));
                    const answer = await pc.createAnswer({ offerToReceiveAudio: true, offerToReceiveVideo: true });
                    await pc.setLocalDescription(answer);
                    channelRef.current.send({ type: 'broadcast', event: 'webrtc-answer', payload: { targetEmail: payload.sender, answer: pc.localDescription, sender: userEmail } });
                    const pending = pendingCandidatesRef.current[payload.sender] || [];
                    for (const candidate of pending) { try { await pc.addIceCandidate(new RTCIceCandidate(candidate)); } catch (err) { } }
                    pendingCandidatesRef.current[payload.sender] = [];
                    if (!inCallRef.current) {
                        inCallRef.current = true;
                        setInVoiceCall(true);
                    }
                } catch (err) { console.error("[WebRTC] Auto-accept error:", err); }
            } else {
        ...
        channel.subscribe(async (status) => {
            if (status === 'SUBSCRIBED') {
                try { await channel.track({ email: userEmail, online: true, timestamp: new Date().toISOString() }); } catch (error) { }
            }
        });

        return () => {
            if (channelRef.current) {
                try { channelRef.current.untrack(); supabase.removeChannel(channelRef.current); } catch (error) { }
    ...
    const sendMessage = async (e) => {
        ...
        setChatInput('');
        const { data, error } = await supabase.from('messages').insert([{ sender_email: userEmail, receiver_email: selectedContact, text: textToSend }]).select();
        if (error) { alert(`Error sending message: ${error.message}`); return; }
        if (data && data.length > 0) {
            setChatMessages(prev => { if (prev.find(m => m.id === data[0].id)) return prev; return [...prev, data[0]]; });
        }
    };
    ...
    const handleImportContacts = async () => {
        ...
        try {
            ...
            if (supported) {
                ...
                } catch (err) { alert("Contact selection failed."); setIsImporting(false); return; }
            } else {
                const emailInput = prompt("Enter an email address to send an invite manually:");
                if (emailInput && emailInput.trim()) {
                    const targetEmail = emailInput.trim();
                    if (!targetEmail.includes('@') || !targetEmail.includes('.')) { alert("Invalid email."); setIsImporting(false); return; }
            ...
            contactsToProcess.forEach(contact => {
                ...
                if (!existingLocalEmails.has(emailLower)) contactsToAdd.push({ name: contact.name || contact.email.split('@')[0], email: contact.email.trim() });
            });
            if (contactsToAdd.length > 0) {
                setSavedContacts(prev => { const merged = [...prev, ...contactsToAdd]; localStorage.setItem('totalRecallContacts', JSON.stringify(merged)); return merged; });
            }
            for (const contact of contactsToProcess) {
                try { await supabase.functions.invoke('send-email', { body: { to: contact.email, subject: `📱 ${displayName} wants to connect on TotalRecall!`, html: generatePrettyEmailHTML(contact.name, displayName, userEmail) } }); } catch (error) { }
            }
            alert(`Processed ${contactsToProcess.length} contacts successfully.`);
        } catch (error) { alert("Error importing contacts."); }
    ...
    const handleRemoveContact = (e, emailToRemove) => {
        ...
        if (window.confirm('Remove this contact?')) {
            setSavedContacts(prev => { const updatedContacts = prev.filter(c => c.email !== emailToRemove); localStorage.setItem('totalRecallContacts', JSON.stringify(updatedContacts)); return updatedContacts; });
            if (selectedContact === emailToRemove) setSelectedContact(null);
        }
    };

    const refreshPresence = async () => {
        if (channelRef.current) { try { await channelRef.current.track({ email: userEmail, online: true, timestamp: new Date().toISOString() }); } catch (error) { } }
    };
                    ...
                    <div style={{ width: isMobile ? '100%' : '30%', minWidth: isMobile ? '100%' : '250px', borderRight: isMobile ? 'none' : '1px solid #222d34', display: 'flex', flexDirection: 'column', backgroundColor: '#111b21', height: '100%', overflow: 'hidden' }}>
                        <div style={{ padding: '15px', backgroundColor: '#202c33', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                <div style={{ width: '36px', height: '36px', borderRadius: '50%', backgroundColor: '#00a884', display: 'flex', justifyContent: 'center', alignItems: 'center', color: '#111', fontWeight: 'bold' }}>{displayName.charAt(0).toUpperCase()}</div>
                        ...
                        <div style={{ flexGrow: 1, overflowY: 'auto' }}>
                            ...
                            {isOnlineExpanded && (onlineUsers.length === 0 ? <div style={{ padding: '20px', textAlign: 'center', color: '#8696a0', fontSize: '14px' }}>No users online.</div> : onlineUsers.map(u => {
                                ...
                                return <div key={u.email} onClick={() => setSelectedContact(u.email)} style={{ padding: '15px', cursor: 'pointer', display: 'flex', alignItems: 'center', borderBottom: '1px solid #222d34', backgroundColor: selectedContact === u.email ? '#2a3942' : 'transparent' }}>
                            ...
                            {isMembersExpanded && (displayMembers.length === 0 ? <div style={{ padding: '20px', textAlign: 'center', color: '#8696a0', fontSize: '14px' }}>No members found.</div> : displayMembers.map(c => <div key={c.email} onClick={() => setSelectedContact(c.email)} style={{ padding: '15px', cursor: 'pointer', display: 'flex', alignItems: 'center', borderBottom: '1px solid #222d34', backgroundColor: selectedContact === c.email ? '#2a3942' : 'transparent' }}>
                            ...
                            {isContactsExpanded && (displayLocalContacts.length === 0 ? <div style={{ padding: '20px', textAlign: 'center', color: '#8696a0', fontSize: '14px' }}>No contacts added yet.</div> : displayLocalContacts.map(c => <div key={c.email} onClick={() => setSelectedContact(c.email)} style={{ padding: '15px', cursor: 'pointer', display: 'flex', alignItems: 'center', borderBottom: '1px solid #222d34', backgroundColor: selectedContact === c.email ? '#2a3942' : 'transparent' }}>
                            ...
                            <>
                                ...
                                {inVoiceCall && (
                                    <div style={{ height: '45vh', backgroundColor: '#000', display: 'grid', gridTemplateColumns: `repeat(${Object.keys(remoteStreams).length + 1}, 1fr)`, gap: '10px', padding: '10px', borderBottom: '1px solid #222d34', flexShrink: 0 }}>
                                        ...
                                        {Object.entries(remoteStreams).map(([email, stream]) => <RemoteVideo key={email} stream={stream} email={email} allKnownUsers={allKnownUsers} />)}
                                    </div>
                                )}
                                ...
                                <div ref={chatContainerRef} style={{ flexGrow: 1, padding: '20px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '8px', backgroundImage: 'url(https://user-images.githubusercontent.com/15075759/28719144-86dc0f70-73b1-11e7-911d-60d70fcded21.png)', backgroundSize: 'contain' }}>
                                    {chatMessages.map((m, i) => {
                                        ...
                                        return <div key={m.id || i} style={{ alignSelf: isMine ? 'flex-end' : 'flex-start', backgroundColor: isMine ? '#005c4b' : '#202c33', padding: '8px 12px', borderRadius: '8px', maxWidth: '65%', fontSize: '14.5px', boxShadow: '0 1px 0.5px rgba(11,20,26,.13)', wordWrap: 'break-word', whiteSpace: 'pre-wrap' }}><div style={{ wordWrap: 'break-word', whiteSpace: 'pre-wrap' }}>{m.text}</div></div>;
                                    })}
                                </div>
    ...
    const handleAuth = async (e, type) => {
        ...
        try {
            if (type === 'login') {
                ...
                if (error) throw error;
                if (data?.user) { await supabase.from('profiles').upsert([{ email: email, name: email.split('@')[0] }]); }
                if (data?.user && !data?.session) {
                    try { await supabase.functions.invoke('confirm-email', { body: { to: email, name: email.split('@')[0], confirmLink: `${window.location.origin}/verify` } }); } catch (invokeErr) { }
